...
class TurtleSpawner(Node):
    def __init__(self):
        ...
        self.timer_ = self.create_timer(100.0, self.call_spawner_server)
        self.turtle_arr = []
    ...
    def callback_catch_turtle(self, request, response):
        ...
        kill_request.name = request.name # Send a request to Kill on TurtleSim based on service request

        future = self.client.call_async(self.request)

        while rclpy.ok():
            rclpy.spin_once(self) # continue other service while waiting .  . . 
            if future.done():
                if future.result() is not None:
                    ...
                    msg.turtle_array = self.turtle_arr
                    self.turtle_publisher_.publish(msg)
                    response = True # Kill was succesful
                else:
                    self.get_logger().warning('Service call failed')
                    response = False # Kill was unsuccesful
                break
